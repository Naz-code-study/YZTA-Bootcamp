...
const SavedRecommendationsScreen = () => {
  ...
  const savedItems = useMemo(() => getSavedRecommendations(activeFilter), [activeFilter]);
    ...
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          return (
            <TouchableOpacity onPress={() => setActiveFilter(item.id)} activeOpacity={0.8}>
              {isActive ? (
                <LinearGradient
      ...
      {savedItems.length > 0 ? (
        ...
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptyTitle}>Henüz bu kategoride bir öneri kaydetmedin</Text>
